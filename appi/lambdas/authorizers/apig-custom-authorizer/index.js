...
const oktaJwtVerifier = new OktaJwtVerifier({
  ...
  assertClaims: {
    aud: 'api://default',
    ...(['qa', 'prod'].includes(process.env.stage) && {cid:process.env.cid})
  }
});
...
exports.handler = async (event, context, callback) => {
    ...
    try {
        jwt = await oktaJwtVerifier.verifyAccessToken(event.authorizationToken.replace(/^Bearer /, ""));
        
        console.log(jwt);
    } catch (error) {
        console.log('okta verification error', error);
    }
    ...
    const principalId = jwt.claims.uid;
    ...
    authResponse.context = {
        ...
        accessToken: event.authorizationToken.replace(/^Bearer /, "")
    };
